...
export default function Summary(props: Props) {
	...
	async function handle_like() {
		if (!token) {
			...
			document.cookie = `redirect_to=${encoded_url}; path=/login; max-age=3600; SameSite=strict; Secure;`
			document.cookie = `redirect_action=like summary ${ID}; path=${window.location.pathname}; max-age=3600; SameSite=strict; Secure`
